...
void ui::RenderUI(Aimbot *aimbot, ESP *esp, Server *server) noexcept
{
    ...
    if(esp->enabled){
        for(int i = 0; i < Server::MAX_PLAYERS; i++){
            ...
            ImVec2 pos = {((player.position.x - server->localPlayer.position.x)+1920/2)-player.player_size/2, ((player.position.y - server->localPlayer.position.y)+1080/2)-player.player_size/2};
            if(Server::GetValidPlayer(player) && player.id != server->localPlayer.id && (player.id > 0 || player.id < Server::MAX_PLAYERS)){           
                ImGui::GetBackgroundDrawList()->AddRect(pos, { pos.x+player.player_size, pos.y+player.player_size }, esp->color_box, 0, 0, 1);
                ImGui::GetBackgroundDrawList()->AddText({pos.x, pos.y-20}, esp->color_name, std::to_string(player.id).c_str());
                ImGui::GetBackgroundDrawList()->AddLine({1920/2, 1080/2}, {pos.x+player.player_size/2, pos.y+player.player_size/2}, esp->color_line, 1);
                char dist[100];
                sprintf(dist, "Distance: %i", (int)Aimbot::distance(player.position, server->localPlayer.position));
                ImGui::GetBackgroundDrawList()->AddText({pos.x, pos.y-30}, esp->color_name, dist);
            }else if(player.id == server->localPlayer.id && esp->show_nearest){
                aimbot->GetNearestToPlayer(9999, server);
                char dist[100];
                sprintf(dist, "Nearest player distance: %i", (int)Aimbot::distance(aimbot->nearestPlayer.position, server->localPlayer.position));
